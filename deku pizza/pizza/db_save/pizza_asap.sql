...
--
-- Host: 127.0.0.1
-- Generation Time: Nov 25, 2019 at 06:01 PM
...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `getmenu` (IN `id` INT)  SELECT *  from menu where id= id$$
...
--
DELIMITER $$
CREATE TRIGGER `delete` BEFORE UPDATE ON `admin_notification` FOR EACH ROW INSERT into notificationlogs values('email','description',time_stamp)
$$
DELIMITER ;
...
--
DELIMITER $$
CREATE TRIGGER `insert` BEFORE UPDATE ON `carts` FOR EACH ROW INSERT into logs values(email,id_object	,qty)
$$
DELIMITER ;
...
INSERT INTO `logs` (`email`, `id_object`, `qty`) VALUES
('de@gmail.com', 1, 1),
('de@gmail.com', 2, 2),
('de@gmail.com', 2, 1),
('de@gmail.com', 3, 1);
...
INSERT INTO `notificationlogs` (`email`, `name`, `time`) VALUES
('', '', '0000-00-00 00:00:00'),
('', '', '0000-00-00 00:00:00');
